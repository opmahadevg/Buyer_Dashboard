...
export default function OverviewDashboardContent() {
  ...
  const handleRangeChange = (id: string) => {
    setActiveRange(id);
    setKpiKey(id);
    if (id === 'range-custom') setShowCustom(true);
  ...
  return (
    <div className="px-8 py-8 max-w-screen-2xl mx-auto">
      ...
      {showCustom && (
        <div className="mb-5 flex items-center gap-3 bg-white border border-[var(--border)] rounded-xl px-4 py-3 w-fit animate-slide-down-in">
          ...
          <div className="flex items-center gap-2">
            <input type="date" defaultValue="2025-11-01" className="text-sm border border-[var(--border)] rounded-md px-2 py-1 outline-none focus:ring-2 focus:ring-primary/30 transition-shadow duration-150" />
            <span className="text-[var(--muted-foreground)] text-sm">to</span>
            <input type="date" defaultValue="2026-05-02" className="text-sm border border-[var(--border)] rounded-md px-2 py-1 outline-none focus:ring-2 focus:ring-primary/30 transition-shadow duration-150" />
          </div>
          <button className="px-3 py-1.5 bg-primary text-white text-xs font-medium rounded-md hover:bg-[#2e29c4] active:scale-95 transition-all duration-150">Apply</button>
        </div>
      )}

      {/* KPI Cards — staggered fade-in on range switch */}
      <div key={kpiKey} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-3 gap-4 mb-8 stagger-children">
        <div className="animate-slide-up"><KpiCard label="Total Spend" value={kpi.spend} subValue={kpi.spendSub} trend="up" trendValue="+24.9% vs previous period" icon={<DollarSign size={18} className="text-primary" />} iconBg="bg-[var(--secondary)]" /></div>
        <div className="animate-slide-up"><KpiCard label="Active Sourcing Requests" value={kpi.active} subValue="Products currently in Quoting" trend="neutral" trendValue="Same as last period" icon={<Package size={18} className="text-blue-600" />} iconBg="bg-blue-50" /></div>
        <div className="animate-slide-up"><KpiCard label="Quote Acceptance Rate" value={kpi.acceptance} subValue="Of all received quotes" trend="down" trendValue="-3.1% vs previous period" icon={<TrendingUp size={18} className="text-purple-600" />} iconBg="bg-purple-50" /></div>
        <div className="animate-slide-up"><KpiCard label="Action Required" value={kpi.actionRequired} subValue="Items needing your response" trend="down" trendValue="Needs attention now" alert icon={<AlertTriangle size={18} className="text-red-500" />} iconBg="bg-red-50" /></div>
        <div className="animate-slide-up"><KpiCard label="Avg. Quote Turnaround" value={kpi.turnaround} subValue="From RFQ to first quote" trend="up" trendValue="-0.8 days vs last period" icon={<Clock size={18} className="text-amber-600" />} iconBg="bg-amber-50" /></div>
        <div className="animate-slide-up"><KpiCard label="Orders In Progress" value={kpi.ordersInProgress} subValue="Active production / delivery" trend="neutral" trendValue="On track" icon={<Truck size={18} className="text-teal-600" />} iconBg="bg-teal-50" /></div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-[var(--border)] p-6 transition-all duration-200 hover:shadow-md hover:shadow-black/5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="text-xs text-primary font-medium hover:underline transition-all duration-150">View all</button>
        </div>
        <ActivityFeed />
      </div>
